...
#		print $sub_zone "$name\t\t\t$_->{class}\t$_->{type}\t" . lc($_->{host}) . "\n"; 
	...
	close $sub_zone ;
	 ...
	 @new_zones_lines = ptr_sort(@new_zones_lines);
	 foreach (@new_zones_lines) {
	  print $new_zone "$_";
...
foreach (@_) {
	...
	$sort_hash{$sort_key[0]} = $_ ;
	}	

foreach (@return_2) {push @return, $_ ; } 
	
foreach my $key ( sort {$a<=>$b} keys %sort_hash) {
           push @return, $sort_hash{$key};
}
	return @return ;
}
